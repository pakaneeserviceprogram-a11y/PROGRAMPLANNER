...
>
> **`clientId`** (String?, เพิ่ม 2026-09-18) — นัดที่สร้างจากหน้าลูกค้า เก็บ id ของ `Client` (คือ `linkedEntityId` เวอร์ชันเฉพาะโมดูลลูกค้า)
> ใช้กับ `ScheduleRepository.upcomingForClient` / `nextForClient` เพื่อโชว์ "นัดถัดไป" ในรายชื่อลูกค้า — ผูกด้วย id ไม่ใช่ชื่อ เพราะเปลี่ยนชื่อลูกค้าได้
>
...
### BodyProfile (เพิ่ม 2026-09-19 — box `body_profile`)

ข้อมูลร่างกายที่ผู้ใช้กรอกเอง ใช้คำนวณเป้าหมายโภชนาการเฉพาะบุคคล (`NutritionPlanner`)

| ฟิลด์ | ชนิด | คำอธิบาย |
|---|---|---|
| sex | enum: `male`, `female` | ใช้ในสูตร Mifflin-St Jeor (ค่าคงที่ต่างกัน 166 kcal) |
| birthYear | int? | **ปีเกิด ค.ศ.** ไม่เก็บอายุตรง ๆ เพื่อให้อายุขยับเองทุกปี |
| heightCm | double? | |
| weightKg | double? | |
| activityLevel | enum: `sedentary`…`veryActive` | ตัวคูณ TDEE 1.2–1.9 |
| weightGoal | enum: `lose`, `maintain`, `gain` | ปรับพลังงาน −15% / คงที่ / +10% |

> อยู่ในไฟล์สำรอง (`BackupService._boxNames`) แต่ไม่ถูกล้างตอน "ล้างประวัติทั้งหมด" เพราะเป็นข้อมูลตั้งค่า
> ผลคำนวณไม่ได้เก็บลงฐานข้อมูล — คำนวณใหม่ทุกครั้งจาก `BodyProfile` + นาทีออกกำลังกายจริง

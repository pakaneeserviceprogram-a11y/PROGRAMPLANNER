...
อัปเดตล่าสุด: 2026-09-26 (คำนวณสารอาหารจากวัตถุดิบ) — ใช้ไฟล์นี้เพื่อกลับมาทำงานต่อได้เร็ว ไม่ต้องไล่อ่านทั้งบทสนทนาใหม่
...
### รอบ 2026-09-26 — คำนวณสารอาหารจากวัตถุดิบ (ชั่งเป็นกรัม)

หน้าใหม่ `lib/screens/recipe_calculator_screen.dart` เข้าจากการ์ดในหน้าโภชนาการ

- **`lib/data/ingredient_database.dart`** — วัตถุดิบ ~45 อย่าง **ค่าต่อ 100 ก.** (เนื้อไก่/วัว/หมู/ปลา/ไข่/นม/ผัก/ผลไม้/ข้าว/น้ำมัน)
  - **วิตามินเก็บเป็น % ของที่ควรได้ต่อวันต่อ 100 ก.** (ไม่ใช่ธงว่ามี/ไม่มี) จึงบอกได้ว่า "จานนี้ให้วิตามิน C 85%"
  - ใส่เฉพาะวิตามินที่ให้ตั้งแต่ ~10% ขึ้นไป ต่ำกว่านั้นไม่มีผลจริงและทำให้ตัวเลขรวมเฟ้อ
  - มี `unitHint`/`unitGrams` (ไข่ 1 ฟอง = 50 ก., น้ำมัน 1 ช้อนโต๊ะ = 14 ก.) ใช้เป็นน้ำหนักตั้งต้นตอนกดเพิ่ม
- **`lib/data/recipe_calculator.dart`** — `RecipeItem` (วัตถุดิบ + กรัม) → `RecipeTotals`
  - `energySplit` บอกสัดส่วนพลังงานจากโปรตีน/คาร์บ/ไขมัน (ดูง่ายกว่ากรัมดิบว่าจานนี้มันเกินไหม)
  - `toMealEntry` บันทึกเป็นมื้ออาหารได้ โดย**ติดธงวิตามินเฉพาะตัวที่ได้ถึง 15% ของวัน** ไม่งั้นการ์ดวิตามินบนหน้าโภชนาการจะหลอกตัวเอง
- **ที่มาของตัวเลข**: ค่ากลางจากตารางคุณค่าอาหารมาตรฐาน (แนว USDA FoodData Central ซึ่งเป็นสาธารณสมบัติ) ปัดให้จำง่าย — เขียนข้อจำกัดไว้ทั้งในโค้ดและท้ายหน้าจอ
- **ถ้าจะต่อฐานข้อมูลจริง**: USDA FoodData Central มี API ฟรี (ต้องสมัคร key) / Open Food Facts ไม่ต้องใช้ key แต่เน้นอาหารในซอง / ตารางอาหารไทยของ ม.มหิดล ตรงกับอาหารไทยที่สุดแต่ไม่มี API เปิดและต้องเช็คลิขสิทธิ์
- test ใหม่ `test/recipe_calculator_test.dart` (16 tests) รวมการตรวจความสมเหตุสมผลของข้อมูล (พลังงานต้องสอดคล้องกับสารอาหาร ±25%) — รวม **266 tests**

### รอบ 2026-09-23 — แก้สแกนนามบัตร error เฉพาะ release build (1.8.1+10)

ผู้ใช้รายงานว่าสแกนนามบัตรบนมือถือแล้ว error (debug build ไม่เคยเจอ)

- **สาเหตุ**: R8 **เปลี่ยนชื่อคลาสของ ML Kit** ตอน minify — ตรวจด้วยการค้น dex ใน APK 1.8.0 พบว่า
  `com/google/mlkit/vision/text/latin`, `MlKitException`, `TextRecognizerOptionsInterface` มี **0 ครั้ง**
  แต่ ML Kit โหลดโมดูลด้วยชื่อคลาสตอนรันจริง จึงหาไม่เจอแล้วโยน error
- **แก้**: เพิ่ม `-keep class com.google.mlkit.** { *; }` (+ ของปลั๊กอิน) ใน `android/app/proguard-rules.pro`
  — ของเดิมมีแต่ `-dontwarn` ซึ่งแค่ปิดคำเตือนตอน build **ไม่ได้กันการตัด/เปลี่ยนชื่อคลาส**
- ยืนยันหลังแก้: สตริงทั้งสามกลับมาอยู่ใน dex แล้ว
- **ยังไม่ได้ทดสอบบนเครื่องจริง** — emulator ค้างที่ขั้น Vulkan หลังเครื่องรีสตาร์ต (Windows Update) จึงทำซ้ำไม่ได้
